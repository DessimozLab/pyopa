...
def align(s1, s2, env):
    return swps3_alignScalar(env.matrix, s1, len(s1), s2, len(s2), env.gapOpen, env.gapExt, env.threshold)


def normlaizedAlign(s1, s2, env):
    return align(normalizeString(s1), normalizeString(s2), env)
...
class AlignmentEnvironment:
    def __init__(self):
        ...
        self.matrix = np.ndarray(shape=(26, 26), dtype=np.float64)
    ...
    def readFromFile(self, file):
        ...
        self.matrix = np.array(extendedMatrix, dtype=np.float64)
        n = 26
